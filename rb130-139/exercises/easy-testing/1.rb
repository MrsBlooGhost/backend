...
# `#assert(test, msg = nil)`
  # `#assert` tests whether its first argument is truthy. If it isn't, the test fails, and the second argument is displayed as the failure message. The default message for `#assert` lacks context, so it's common to specify the failure message when using `#assert`.
